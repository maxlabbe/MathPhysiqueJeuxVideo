...
// ctor
///////////////////////////////////////////////////////////////////////////////
Sphere::Sphere(float radius, Vector3D* position, Vector3D* color, int sectors, int stacks, bool smooth) : interleavedStride(32)
{
    set(radius, sectors, stacks, smooth);
    this->position = position;
    this->color = color;
    int glvc_size = getVertexCount() * 6;
    this->glVerticesColor = new GLfloat[glvc_size];
    this->glIndices = new GLuint[getIndexCount()];
    updateGLVerticesColor(); // white, default color
    generateGLIndices();
}
...
// special getter 
///////////////////////////////////////////////////////////////////////////////
void Sphere::updateGLVerticesColor()
{
    unsigned int count_to_3 = 1;
    unsigned int ind = 0;
    for (auto it = std::begin(vertices); it != std::end(vertices); ++it)
    {
        if ( count_to_3 == 1 )
        {
            this->glVerticesColor[ind++] = GLfloat(*it + position->getX());
            count_to_3++;
        }
        else if (count_to_3 == 2)
        {
            this->glVerticesColor[ind++] = GLfloat(*it + position->getY());
            count_to_3++;
        }
        else
        {
            this->glVerticesColor[ind++] = GLfloat(*it + position->getZ());
            this->glVerticesColor[ind++] = GLfloat(color->getX());
            this->glVerticesColor[ind++] = GLfloat(color->getY());
            this->glVerticesColor[ind++] = GLfloat(color->getZ());
            count_to_3 = 1;
        }
    }
}

void Sphere::generateGLIndices()
{   
    const unsigned int* inds = getIndices();
    int i = 0;
    for (auto it = std::begin(indices); it != std::end(indices); ++it)
    {
        this->glIndices[i++] = GLuint(*it);
    }
}
...
{
    this->color->set(color.getX(), color.getY(), color.getZ());
}
